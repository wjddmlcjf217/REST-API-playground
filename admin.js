...
// inventory display method to edit
function getAllToEdit() {
  resource = "herbs/";
  xhttp.open("GET", endPointRoot + resource, true);
  xhttp.send();
  xhttp.onreadystatechange = function () {
...
// returns hashcode which references id of item to be editted
function displayHashCode() {
...
// table generation for inventory display
function displayHerbsToEdit(obj) {
  console.log(obj)
  let table = document.createElement('table');
  let th1 = document.createElement('th')
  let th2 = document.createElement('th')
  ...
  for (let i = 0; i < obj.length; i++) {
    ...
    let nameEditFieldBtn = document.createElement('button')
    nameEditField.setAttribute("onclick", 'retrieveID()');
    nameEditFieldBtn.setAttribute("onclick", 'retrieveID()');
